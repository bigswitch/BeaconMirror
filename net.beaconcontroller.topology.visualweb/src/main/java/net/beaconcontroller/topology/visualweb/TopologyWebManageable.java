...
public class TopologyWebManageable implements IWebManageable, ITopology /*ITopologyAware*/ {
  ...
  public TopologyWebManageable() {
    ...
    tabs.add(new Tab("Topology Viewer", "/wm/topology/topoview.do"));
  ...
  public View getTopologyJson(Map<String,Object> model) {
    ...
    Map<Long, Set<LinkTuple>> switchLinks = topologyProvider.getSwitchLinks();
    for(Map.Entry<Long, Set<LinkTuple>> e : switchLinks.entrySet()) {
      ...
      for(LinkTuple lt : links) {
        ...
        String srcID = "" + lt.getSrc().getId();
        String dstID = "" + lt.getDst().getId();
